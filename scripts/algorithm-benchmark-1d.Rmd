...
```{r}
algorithms <- list(
    dens_int = function(X, Y) kld_est_kde1(X = X, Y = Y),
    dens_mc  = function(X, Y) kld_est_kde1(X = X, Y = Y, MC = TRUE),
    nn_1     = kld_est_1nn,
    nn_k_gen = function(X, Y) kld_est_gknn(X = X, Y = Y, warn.max.k = FALSE)
)
...
generally fastest. All investigated methods scale approximately linearly with 
sample size, which is due to the use of a fast Fourier transform in kernel density
estimation and use of the kd-tree in the nearest neighbours search. The generalized
kernel density estimator `nn_k_gen` is approximately 1 order of magnitude slower than the 
1-nearest neighbour estimator `nn_1`, without offering additional accuracy in the
1D examples. The extra effort starts to pay off in higher-dimensional problems.
