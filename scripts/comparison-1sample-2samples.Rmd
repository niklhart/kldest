...
I haven't yet fully understood why this is the case. Interestingly, independent
of sample size and of the underlying distribution, the bias is always approximately
$+0.6$.
...
for (i in 1:nRep) {
    ...
    Y <- Yfun()
    kld_XY[i] <- kld_est_1nn(X, Y)
    kld_Xq[i] <- kld_est_knn_1sample(X, q, k = 2)
}
# graphics
boxplot(cbind(XY = kld_XY, Xq = kld_Xq), main = "KL divergence (Gaussian model)")
abline(h = kld_true, lty = 2)
...
$\Rightarrow$ only getting workse with $k>1$...
