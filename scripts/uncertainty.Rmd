...
for (i in 1:nscenario) {
    ...
    for (j in 1:nAlgo) {
        kldboot <- kld_ci_subboot(X, Y, estimator = algorithms[[j]], B = 200L, size = sqrt)
...
coverage <- reshape2::dcast(results, sample.size + algorithm + distribution ~ ., 
                value.var = "covered", fun.aggregate = mean)
...
for (i in 1:nRep) {
    ...
    for (j in 1:nRep) {
        ...
        kld_approx[j] <- algorithms$nn_1(Xi, Yij)
    }
...
$\Rightarrow$ in this easy example, coverage is quite good (indeed, better than
the one obtained using the subsampling bootstrap!)... maybe combine the two? 
I.e., subsampling bootstrap for `X` and sampling from the fitted distribution for 
`Y`?
