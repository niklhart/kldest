---
title: "Algorithm performance in large dimensions (one sample)"
...
q <- function(x) dexp(x, rate = lambda2, log = FALSE)
...
for (i in seq_along(n)) {
    for (j in 1:nrep) {
        ...
        kld[j,i] <- kld_est_nn(X, q = q, log.q = FALSE, eps = 0)
    }
}
```
...
plot(x = n, y = kld_mean, type = "l", log = "x", ylab = "KL divergence",
     ...
     ylim = range(c(kld_mean, kld_true), finite = TRUE))
...
q <- function(x) mvdnorm(x, mu = paramApprox$mu, Sigma = paramApprox$sigma)
```
...
for (i in seq_along(n)) {
    for (j in 1:nrep) {
        ...
        kld[j,i] <- kld_est_nn(X, q = q, eps = 0)
    }
}
```
...
for (i in seq_along(n)) {
    for (j in 1:nrep) {
        ...
        kld1[j,i] <- kld_est_nn(X, q = q, eps = 0)
...
abline(h = kld_true, lty = 2)
legend("bottomright", 
       legend = c("plain nn (1 sample)","bias-reduced nn (2 samples)","True KL-Div."),
       lty = c(1,1,2),
